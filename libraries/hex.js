...
export class Hex 
{
    ...
    static hexToPixel(point2D) {
        ...
        let y = size * (Math.sqrt(3)/2 * point2D.x  +  Math.sqrt(3) * point2D.y) + (size * Math.sin(Math.PI/3)) + origo.y - Math.floor(point2D.x/2)*size*Math.sqrt(3); // creating an y-basis vector from origin to the hexagon that is clicked, giving its central y- coordinate
